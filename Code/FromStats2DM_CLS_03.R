...
chart.Correlation(iris[-5], pch=21)
...
###################################
# Reading student data (CSV file) #
###################################

library(readr)
# I have the file "student_data.csv" in a repository "data"
# which is in the repository "R" on "MyDocs" (default repository)
students <- read.csv("~/R/data/student_data.csv",
                       header = TRUE, sep = ",", 
                       quote = "\"'", dec = ".")

row.names(students) <- students[,1]
...
students[,6] <- 2021 - students[,2]

colnames(students) <- c(colnames(students[1:5]),"Age")

cor(students)
# Significant correlation only
# between the variables Age and NbDays (98.93%)

nb_students <- nrow(students)

x <- seq(1:nb_students)
# They are 16 students
...
# How many students do you expect to find with a size  
# greater than 180?
...
library(xtable)
s<-summary(students)
tab<-xtable(s, caption= "summary statistics on the students", 
